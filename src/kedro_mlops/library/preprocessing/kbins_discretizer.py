...
class KBinsDiscretizer(BaseEstimator):
    ...
    though it is purely written in pandas instead of numpy because it is more intuitive.
    ...
    def set_attributes_from_dict(self, params: dict):
        ...
        if not isinstance(_bins_by_column, dict):
            raise ValueError(
                "_bins_by_column is expected to be a dict "
                f"but is of type {type(_bins_by_column)} instead"
            )
    ...
    def fit(self, data: pd.DataFrame, column_names: list):
        ...
        for column_name in column_names:
            ...
            if column_name not in data.columns:
                log.warning(
                    f"DataFrame has no column '{column_name}', so it will be "
                    "skipped in fitting"
                )
                continue
    ...
    def _fit_column(self, data: pd.DataFrame, column_name: str) -> list[tuple]:
        ...
        if col_min == col_max:
            log.warning(
                f"Predictor '{column_name}' is constant and "
        ...
        if len(bin_edges) < 3:
            log.warning(
                f"Only 1 bin was found for predictor '{column_name}' so it will "
    ...
    def transform(self, data: pd.DataFrame, column_names: list) -> pd.DataFrame:
        ...
        for column_name in column_names:
            if column_name not in self._bins_by_column:
                log.warning(
                    f"Column '{column_name}' is not in fitted output "
                    "and will be skipped"
                )
                continue
    ...
    def _compute_bin_edges(
        ...
        if self.strategy == "quantile":
            bin_edges = list(
                data[column_name].quantile(
                    np.linspace(0, 1, n_bins + 1), interpolation="linear"
                )
            )
    ...
    def _create_index(
        ...
        if closed not in ["left", "right"]:
            raise ValueError(
                ...
                "Got strategy={!r} instead.".format(
                    KBinsDiscretizer.__name__, ["left", "right"], closed
                )
            )
    ...
    def _create_bin_labels(self, bins: list[tuple]) -> list:
        ...
        for interval in bins:
            bin_labels.append(self.label_format.format(interval[0], interval[1]))

        # Format first and last bin as < x and > y resp.
        if self.change_endpoint_format:
            if self.closed == "left":
                bin_labels[0] = f"< {bins[0][1]}"
                bin_labels[-1] = f">= {bins[-1][0]}"
            else:
                bin_labels[0] = f"<= {bins[0][1]}"
                bin_labels[-1] = f"> {bins[-1][0]}"
